...
/* harmony import */ var _modules_models__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./modules/models */ "./src/js/modules/models.js");
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  Object(_modules_models__WEBPACK_IMPORTED_MODULE_0__["default"])();
  ...
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="name"]'); // для ввода имени
...
var accordion = function accordion(triggersSelector) {
  ...
  btns.forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      this.nextElementSibling.classList.toggle("active-content"); // получаем следующий элемент и тоглим класс
  ...
  //         // есть ли у элемента, на который мы нажали, nety классa active
  //         btns.forEach((btn) => {
...
var burger = function burger(menuSelector, burgerSelector) {
  ...
  burgerElem.addEventListener('click', function () {
    if (menuElem.style.display == 'none' && window.screen.availWidth < 993) {
      // если у нас меню скрыто и ширина экрана меньше 993
      menuElem.style.display = 'block'; // мы его показываем
    } else {
      // но если, оно изначально показывалось, мы его скрываем
      menuElem.style.display = 'none';
    }
  });
  window.addEventListener('resize', function () {
    // событие отслеживание изменения размера экрана
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunc = function calcFunc() {
    sum = Math.round(+sizeBlock.value * +materialBlock.value + +optionslBlock.value); // В переменную sum присваиваем округленное значание-формулу от заказчика(т.к приходит ввиде строки ставим +)

    if (sizeBlock.value == "" || materialBlock == "") {
      // Если пользователь не заполнил первый или второй select
      resultlBlock.textContent = "Пожалуйста, выберете размер и материал картины"; // выводим подсказку
...
var drop = function drop() {
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener("drop", function (e) {
      ...
      var dots; // это перменная будет содержать троеточие либо нечего
      ...
      var name = arr[0].substring(0, 6) + dots + arr[1]; // вырезаем кусок строки от 0 до 5 символа + троекточие / точка + расширение
...
/* harmony default export */ __webpack_exports__["default"] = (filter); // ЛИБО ЖЕ ТАК БОЛЕЕ КОРОТКО
// const filter = () => {
...
var forms = function forms() {
  var form = document.querySelectorAll("form"),
      ...
      upload = document.querySelectorAll('[name="upload"]'); //   checkNumInputs('input[name="user_phone"]'); // валидация инпутов
  ...
  form.forEach(function (item) {
    ...
    item.addEventListener("submit", function (e) {
      ...
      var statusMessage = document.createElement("div"); // start Создаем блок для показа статуса отправки пользователю
      ...
      var statusImg = document.createElement("img"); // добавляем img
      ...
      var textMessage = document.createElement("div"); // добавляем элемент див на страницу
      ...
      var formData = new FormData(item); // собираем все данные из этой формы

      var api;
      item.closest(".popup-design") || item.classList.contains("calc_form") ? api = path.designer : api = path.question; // если класс .popup-design есть у родителя или есть класс calc_form,  то оправляем по  пути path.designer, если нету то по  path.question

      console.log(api);
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData) // отправляем запрос на сервер по вот этому адресу и данными formData
      .then(function (res) {
        console.log(res);
...
var mask = function mask(selector) {
  var setCusorPosition = function setCusorPosition(pos, elem) {
...
var modals = function modals() {
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      var display;
      document.querySelectorAll("[data-modl]").forEach(function (item) {
  ...
  function openByScroll(selector) {
    window.addEventListener("scroll", function () {
      var scrollHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight); // для древних браузеров
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function hideImg(block) {
    ...
    block.querySelectorAll('p:not(.sizes-hit)').forEach(function (p) {
      p.style.display = 'block'; // p.remove()
    });
  }

  blocks.forEach(function (block) {
    block.addEventListener('mouseover', function () {
      // событие когда мышь находить над элементом
      showImg(block);
    });
  ...
  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      ...
      var widthTop = document.documentElement.scrollTop,
          ...
          // вверхняя граница, до куда будем скроллить(header)
      start = null;
      requestAnimationFrame(step);

      function step(time) {
        if (start === null) {
          // если анимация запускаеться первый раз
          start = time;
        }
